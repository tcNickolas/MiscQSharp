...
namespace Microsoft.Quantum.Samples.SudokuGrover {
    ...
    : (Bool, Int[]) {
        ...
        // Note that startingNumberConstraints are only used in the state preparation routine, not in the oracle itself!
        ...
        Message($"   emptySquareEdges ({Length(emptySquareEdges)}) = {emptySquareEdges}");
        Message($"   startingNumberConstraints ({Length(startingNumberConstraints)}) = {startingNumberConstraints}");
    ...
    /// # Summary
    /// Evaluate search space size based on the constraints.
